'use client';

import { motion } from 'framer-motion';

export default function CTASection() {
  return (
    <section id="reservar" className="py-32 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">

        <motion.div
          initial={{ opacity: 0, y: 24 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.7 }}
          className="relative rounded-[3rem] overflow-hidden shadow-2xl"
        >
          {/* Background image */}
          <img
            src="/fina.jpg"
            alt="Mar Caribe"
            className="absolute inset-0 w-full h-full object-cover"
          />
          {/* Dark overlay */}
          <div className="absolute inset-0 bg-gradient-to-br from-slate-950/85 via-slate-900/75 to-teal-950/80" />

          {/* Content */}
          <div className="relative z-10 p-10 md:p-20 grid lg:grid-cols-2 gap-12 items-center">

            <div>
              <span className="text-teal-400 font-medium tracking-[0.35em] uppercase text-xs mb-5 block">
                Únete a nosotros
              </span>
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-white leading-tight mb-6">
                ¿LISTO PARA TU <br />
                <span className="text-teal-400">PRÓXIMA AVENTURA?</span>
              </h2>
              <p className="text-white/60 text-base md:text-lg font-light leading-relaxed">
                Reserva tu cupo ahora por WhatsApp y asegura la mejor tabla para tu visita.
                Sin prepagos, confirmación inmediata.
              </p>
            </div>

            <div className="flex flex-col sm:flex-row gap-4 lg:justify-end">
              {/* Primary — ghost with teal border */}
              <a
                href="https://wa.me/573125971913"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2.5 border border-teal-400/60 hover:border-teal-400 text-teal-400 hover:text-white hover:bg-teal-500/20 backdrop-blur-sm px-8 py-4 rounded-full font-bold tracking-widest text-xs uppercase transition-all duration-300"
              >
                <WhatsAppIcon className="w-4 h-4" />
                Reservar por WhatsApp
              </a>

              {/* Secondary — ghost white */}
              <button
                onClick={() => document.getElementById('experiencia')?.scrollIntoView({ behavior: 'smooth' })}
                className="inline-flex items-center justify-center gap-2 border border-white/20 hover:border-white/50 text-white/60 hover:text-white backdrop-blur-sm px-8 py-4 rounded-full font-medium tracking-widest text-xs uppercase transition-all duration-300"
              >
                Ver más detalles
              </button>
            </div>

          </div>
        </motion.div>

        {/* Footer subtle brand */}
        <div className="mt-16 text-center">
          <p className="text-slate-300 font-light tracking-[0.5em] text-[10px] uppercase">
            Coveñas Paddle Bay — La experiencia definitiva
          </p>
        </div>

      </div>
    </section>
  );
}

function WhatsAppIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" {...props}>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}
